...
def start_libradtran(sod, ts, lat, lon, alt, atmos_file, ozone, date, channel):
...
with concurrent.futures.ProcessPoolExecutor(max_workers=16) as executor:
    for i in tqdm(range(0, len(array_sod))):
        for channel in channels:
            ...
            future = executor.submit(start_libradtran, array_sod[i], ts[i], array_lat[i], array_lon[i], array_alt[i]/1e3, atmos_files[i], 375, fnum, channel)
